 ...
 * $Revision: 2 $
 * $Date: 16/02/24 15:51 $
 * @brief    PN020 series TIMER driver header file
 *
 * @note
...
/** @addtogroup PN020_TIMER_EXPORTED_CONSTANTS TIMER Exported Constants
  @{
*/
...
#define	TIMER_CAPTURE_SOURCE_EXT_PIN				(0UL)
#define	TIMER_CAPTURE_SOURCE_ACMP_OUTPUT		(1UL << TIMER_CTL_CAPSRC_Pos)
#define	TIMER_CAPTURE_ACMPSSEL_0						(0UL)
#define	TIMER_CAPTURE_ACMPSSEL_1						(1UL << TIMER_EXTCTL_ACMPSSEL_Pos)

/*@}*/ /* end of group PN020_TIMER_EXPORTED_CONSTANTS */
...
/**
  ...
  * @param[in] The capture source for timer capture functions
	*					1 TIMER_CAPTURE_SOURCE_EXT_PIN
	*					2 TIMER_CAPTURE_SOURCE_ACMP_OUT
  * @return None
  */
__STATIC_INLINE void TIMER_SetCaptureSource(TIMER_T *timer,uint32_t Source)
{
    timer->CTL = (timer->CTL & ~TIMER_CTL_CAPSRC_Msk) | Source;
}

/**
  * @brief This function select the Timer capture ACMP Source.
  * @param[in] The number of ACMP for Timer capture functions.
	*					1 TIMER_CAPTURE_ACMPSSEL_0
	*					2 TIMER_CAPTURE_ACMPSSEL_1
  * @return None
  */
__STATIC_INLINE void TIMER_SelSourceACMP(TIMER_T *timer,uint32_t ACMPSel)
{
    timer->EXTCTL = (timer->EXTCTL & ~TIMER_EXTCTL_ACMPSSEL_Msk) | ACMPSel;
}
