 ...
 * $Revision: 2 $
 * $Date: 16/02/24 1:43p $
 * @brief    PN020 series PWM driver source file
 *
 * @note
 ...
 * @param[in] u32DutyCycle Target generator duty cycle percentage. Valid range are between 0 ~ 99. 10 means 10%, 20 means 20%...
 * @return period
                                  ...
                                  uint32_t u32DutyCycle)
{
    ...
    PWM->CTL |= (PWM_CTL_CNTMODE0_Msk << ((4 * u32ChannelNum)));
    PWM->CTL2 = (PWM->CTL2 & ~PWM_CTL2_CNTTYPE_Msk);
    ...
    if(u32DutyCycle == 0)
     ...
     if(u32DutyCycle == 0)
     {
         *((__IO uint32_t *)((((uint32_t) & ((pwm)->PERIOD0)) + (u32ChannelNum) * 4))) = 0;	
         u16CNR = 0;
     }
    else
